        ...
        <div className="px-5 sm:px-10 flex flex-1 justify-center py-5 w-full">
          <div className="layout-content-container flex flex-col w-full py-5 flex-1">
            ...
            <div className="flex justify-between px-4 text-sm text-center items-center">
              ...
              <p className="text-slate-600 text-sm font-normal leading-normal px-4 underline">Forgot Password?</p>
            </div>
            <div className="w-full px-4 py-3">
              ...
              >
                ...
                <div className="text-gray-800 tracking-wide">Sign in with Google</div>
              </a>
            </div>
            <div className="flex justify-center mx-auto w-full px-4">
              <button
                disabled={isPending}
                className="flex cursor-pointer items-center justify-center overflow-hidden rounded-xl h-10 w-full bg-[#1980e6] text-white text-sm font-bold leading-normal tracking-[0.015em]"
                onClick={signIn}
              >
                <span className="truncate">{isPending ? "Signing In..." : "Sign In"}</span>
              </button>
            </div>
            {isError && <p className="text-red-500 text-center mt-2">Login failed. Please try again.</p>}
            {isError && <p className="text-red-500 text-center mt-2">{errorMessage}</p>}
          </div>
        </div>
      </div>
    </div>
